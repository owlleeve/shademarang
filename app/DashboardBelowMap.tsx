...
export default function DashboardBelowMap({ selected, onSelect, knowledge, actions, internal, role, onKnowledge, onAction, onEvaluation, onAI, onAnalysis }: { selected: string; onSelect: (name: string) => void; knowledge: Knowledge[]; actions: Action[]; internal: boolean; role:string; onKnowledge: (id: string) => void; onAction: (id: string) => void; onEvaluation: () => void; onAI: () => void; onAnalysis: () => void }) {
  ...
  return <div className="dashboard-followup">
    ...
    {role==="bappeda"&&<section className="dashboard-followup-grid" aria-label="Analisis untuk BAPPEDA"><article className="card"><span className="eyebrow">Perencanaan wilayah</span><h2>Bandingkan prioritas</h2><p className="muted">Lihat indikator dan metode sebelum menyusun arah tata ruang atau RTH.</p><button className="btn primary" onClick={onAnalysis}>Buka analisis data →</button></article><article className="card"><span className="eyebrow">Riwayat pelaksanaan DLH</span><h2>Pelajari tindakan sebelumnya</h2>{actions.filter(item=>item.area===selected).map(item=><button key={item.id} className="text-link" onClick={()=>onAction(item.id)}>{item.title} · {item.status} →</button>)}<p className="muted small">Riwayat menjadi rujukan perencanaan, bukan instruksi pelaksanaan otomatis.</p></article></section>}
    {role==="dlh"&&<section className="dashboard-followup-grid" aria-label="Tindak lanjut DLH"><article className="card"><span className="eyebrow">Pelaksanaan RTH</span><h2>Tindakan untuk {selected}</h2>{actions.filter(item=>item.area===selected).map(item=><button key={item.id} className="text-link" onClick={()=>onAction(item.id)}>{item.title} · {item.status} →</button>)}<p className="muted small">Perbarui status pekerjaan dari detail tindakan.</p></article><article className="card"><span className="eyebrow">Evaluasi pasca-mitigasi</span><h2>Catat hasil lapangan</h2><p className="muted">Hubungkan kondisi sebelum dan sesudah tindakan agar pembelajarannya dapat dipakai kembali.</p><button className="btn primary" onClick={onEvaluation}>Buka evaluasi →</button></article></section>}
